          ...
          <div className="grid gap-6 lg:grid-cols-[0.42fr_0.58fr] lg:items-start">
            <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 shadow-sm">
              <div className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">
                Quick flow (top to bottom)
              </div>
              <h4 className="mt-2 text-lg font-semibold text-slate-900">Dispatch workflow flowchart</h4>
              <div className="mt-4 flex flex-col gap-3">
                {[
                  "Check stock visibility",
                  "Classify status",
                  "Dealer check & reallocation",
                  "Book transport time & carrier",
                  "Print doc & PGI in SAP",
                  "Delivery documents & receipt",
                  "Damage report & KPI review",
                ].map((label, index, items) => (
                  <div key={label} className="flex flex-col items-center">
                    <div className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm">
                      <div className="flex items-center gap-2">
                        <span className="flex h-6 w-6 items-center justify-center rounded-full border border-slate-200 bg-slate-100 text-xs font-bold text-slate-600">
                          {index + 1}
                        </span>
                        <span>{label}</span>
                      </div>
