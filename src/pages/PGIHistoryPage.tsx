...
const PGIHistoryPage: React.FC = () => {
  ...
  const [composerRecord, setComposerRecord] = useState<PgiHistoryRow | null>(null);
  const [composerRecipientType, setComposerRecipientType] = useState<"dealer" | "vendor">("dealer");
  ...
  const openComposer = (record: PgiHistoryRow) => {
    setComposerRecord(record);
    setComposerRecipientType(recipientType);
    setComposerSubject(templateSubject || "Missing Delivery Document Follow-up");
    setComposerContent(renderTemplateMessage(record));
  };

  const handleSendComposer = async () => {
    if (!composerRecord) return;
    const toEmail =
      composerRecipientType === "dealer"
        ? dealerEmails[String(composerRecord.dealer || "").trim()] || ""
        : vendorEmailMap.get(String(composerRecord.vendorName || "").trim().toLowerCase()) || "";

    if (!toEmail) {
      toast.error(`No ${composerRecipientType} email found for this row.`);
      return;
    }
    ...
    await sendPgiMissingEmail({
      ...
      to_name:
        composerRecipientType === "dealer"
          ? String(composerRecord.dealer || "Dealer")
          : String(composerRecord.vendorName || "Vendor"),
      subject: composerSubject,
      message: composerContent,
      chassis_number: composerRecord.chassisNumber || "",
      pgi_date: String(composerRecord.pgidate || ""),
      vendor_name: String(composerRecord.vendorName || ""),
      dealer_name: String(composerRecord.dealer || ""),
    });
    ...
    selectedEntries.forEach((record) => {
      ...
      const toName =
        recipientType === "dealer"
          ? String(record.dealer || "Dealer")
          : String(record.vendorName || "Vendor");
      const bucket = groupedByRecipient.get(toEmail) || { toName, records: [] };
      bucket.records.push(record);
      groupedByRecipient.set(toEmail, bucket);
    });

    if (!groupedByRecipient.size) {
      toast.error(`No ${recipientType} email found for selected rows.`);
      return;
    }

    for (const [toEmail, payload] of groupedByRecipient.entries()) {
      const detailLines = payload.records.map(
        (record) =>
          `- Chassis: ${record.chassisNumber || "N/A"}, PGI Date: ${String(record.pgidate || "N/A")}, Vendor: ${String(record.vendorName || "N/A")}`
      );

      const mergedMessage = `${templateBody}
...
${detailLines.join("\n")}`;
      ...
      await sendPgiMissingEmail({
        ...
        chassis_number: payload.records.map((record) => record.chassisNumber || "").join(", "),
        pgi_date: "Multiple",
        vendor_name:
          payload.records.length === 1
            ? String(payload.records[0].vendorName || "")
            : "Multiple",
        dealer_name:
          payload.records.length === 1
            ? String(payload.records[0].dealer || "")
            : "Multiple",
      });

      toast.success(`Email sent to ${toEmail} (${payload.records.length} chassis)`);
    }
  };
    ...
    <div className="space-y-4">
      <Card className="border-border/80 shadow-sm">
        ...
        <CardContent>
                    ...
                    <div className="flex items-end gap-2 overflow-x-auto pb-1">
                      {chartData.map((bar) => (
                        <div key={bar.day} className="flex flex-col items-center gap-1">
                          <span className="text-[10px] font-semibold text-slate-600">{bar.count}</span>
                          <div
          ...
          {composerRecord && (
            <div className="mb-3 space-y-2 rounded-lg border border-slate-200 bg-white p-3">
              <div className="text-sm font-semibold text-slate-700">Send email for {composerRecord.chassisNumber}</div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm text-slate-600">Receiver:</span>
                <select
                  className="h-9 rounded-md border border-input bg-white px-3 text-sm"
                  value={composerRecipientType}
                  onChange={(event) => setComposerRecipientType(event.target.value as "dealer" | "vendor")}
                >
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={() => void handleSendComposer()}>Send</Button>
                <Button size="sm" variant="outline" onClick={() => setComposerRecord(null)}>Cancel</Button>
              </div>
            </div>
          )}
                      ...
                      >
                        ...
                        <TableCell className="rounded-r-lg">
                          <Button
                            ...
                            aria-label={`Compose email for ${record.chassisNumber}`}
                          >
